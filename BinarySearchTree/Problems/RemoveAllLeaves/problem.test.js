...
test("Return a tree with only 1 node left after removing 2 leaves", () => {
  ...
  expect(removeAllLeaves(inputTree)).toEqual(outputTree);
});
/*
...
// Test Case 4:
test("Return a tree with only 4 nodes left after removing 2 leaves", () => {
  const inputTree = constructTree([24, 17, 31, 34, 32, 39]);
  const outputTree = constructTree([24, 17, 31, 34]);
  expect(removeAllLeaves(inputTree)).toEqual(outputTree);
});
/*
...
test("Return a tree with 0 nodes left after removing the root leaf", () => {
  ...
  expect(removeAllLeaves(inputTree)).toEqual(new BST());
});
/*
